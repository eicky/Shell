...
wordpress_path="https://wordpress.org/wordpress-5.2.2.tar.gz"
...
caddy_install(){
	...
	Default_caddy
	#caddy官方脚本(v1.0已不可用,改为下面方法)
	#curl https://getcaddy.com | bash -s personal
	mkdir -p /tmp/caddy
	wget https://github.com/caddyserver/caddy/releases/download/v1.0.4/caddy_v1.0.4_linux_amd64.tar.gz -O /tmp/caddy/caddy_v1.0.4_linux_amd64.tar.gz
	tar -zxvf /tmp/caddy/caddy_v1.0.4_linux_amd64.tar.gz -C /tmp/caddy/
...
ExecStart=/usr/local/bin/v2ray -config ${v2ray_conf}
RestartPreventExitStatus=23
Restart=always
User=root
